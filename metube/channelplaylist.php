<div class="multimedia-content-container playlist-bkg">
    <div class="multimedia-content">
        <div class="go-to-favorites"><!--<a href="channelfavoritelist.php">go to favorites</a>--><a href="<?php echo BASE_PATH . CHANNEL_PATH;?>channel.php?type=favorite<?php echo $suffix2;?>">go to favorites</a></div>
        <div class="playlist-favorite">PLAYLIST</div>
        <?php
        while($row = get_array($result)) {
            $media_id = $playlist_id = $playlist_name = $title = $description = $type = $path = null;
            $thumbnailSrc = "";
            foreach ($row as $key => $value) {
                if($key == 'media_id') $media_id = $value;
                if($key == 'title') $title = $value;
                if($key == 'playlist_id') $playlist_id = $value;
                if($key == 'playlist_name') $playlist_name = $value;
            ...
            <div class="media-item">
                ...
                <div class="text-wrapper">
                    ...
                    ';
                    //modifying playlist and favorite is done only by the channel owner
                    if($channelid == $myID) {
                    echo'
                    <div class="remove-and-add">
                        <div class="remove-from-list">
                            <button class="btn btn-link remove-playlist" type="button" name='.$media_id.' value='.$playlist_id.'>Remove</button>
                        </div>
                        <div class="add-to-favorite">
                            <button class="btn btn-link add-favorite-btn" type="button" name='.$media_id.' value="">Add to favorite</button>
                        </div>
                    </div>
                    ';
                    }
        ...
        if($channelid == $myID) {
            if($users_favorite) {
                ...
                while($row = get_array($users_favorite)) {
                    ...
                    echo '<input type="checkbox" name='. $favorite_name .' value='. $favorite_id .' class="favorite_check">'. $favorite_name .'<br>';
                }
            }
            echo '
...
<script>
    ...
    //only process favorite and playlist when the viewer is the uploader(the channel owner)
